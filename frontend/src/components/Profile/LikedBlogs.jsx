...
const LikedBlogs = () => {
  ...
  useEffect(() => {
    const fetchLikedBlogs = async () => {
      try {
        // For now, fetch all blogs – replace with a dedicated liked blogs endpoint when available
        ...
        setError("Could not load liked blogs. Please try again.");
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="container my-5">
      ...
      {blogs.length === 0 ? (
        <div className="text-center text-muted">
          No liked blogs yet. Start liking some!
        </div>
      ) : (
